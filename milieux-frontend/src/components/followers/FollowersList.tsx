...
const FollowersList = async () => {
  ...
  let followerUsers: z.infer<typeof UserSchema>[] = [];
  const loggedInUser: z.infer<typeof UserSchema> = loggedInUserResponse.user;

  if (loggedInUserResponse.success) {
    const followerUsersResponse = await getUsersByIds(
      loggedInUser.followers || []
    );

    if (followerUsersResponse.success) {
      followerUsers = followerUsersResponse.users;
    }
  }
